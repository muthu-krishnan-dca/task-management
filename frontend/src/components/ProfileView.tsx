  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      ...
      <div className="rounded-3xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800 p-6 sm:p-8 shadow-sm space-y-6">
        ...
        {/* Footer Actions */}
        <div className="pt-4 border-t border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row gap-2.5">
          <button
            type="button"
            onClick={() => setIsEditing(!isEditing)}
            className="flex-1 rounded-lg bg-black dark:bg-white px-4 py-2.5 text-xs font-semibold text-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-100 transition shadow-xs cursor-pointer"
          >
          ...
          <button
            ...
            onClick={() => {
              if (confirm("Are you sure you want to log out?")) {
                import("@/utils/authStore").then(({ logoutUser }) => logoutUser("/login"));
              }
            }}
            className="rounded-lg border border-rose-200 dark:border-rose-800/60 bg-rose-50/50 dark:bg-rose-950/30 px-4 py-2.5 text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/40 transition cursor-pointer flex items-center justify-center gap-1.5"
          >
            <span>🚪</span>
            <span>Log Out</span>
          </button>
